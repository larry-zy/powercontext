...
"""Resolve a stable PowerContext scope for one LangGraph run.

The priority is inverted relative to the Codex plugin. A LangGraph deployment is typically a long-running service in
which the working directory has no relationship to the project, so an explicit scope is the primary path and Git
